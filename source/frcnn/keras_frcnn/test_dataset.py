...
def show_dataset(X, Y, image_data, debug_img, debug_num_pos, anchor_box_scales, anchor_box_ratios, rpn_stride, im_size):
    print('Original image: height=%d width=%d'%(image_data['height'], image_data['width']))
    print('Resized image:  height=%d width=%d C.im_size=%d'%(X.shape[1], X.shape[2], im_size))
    print('Feature map size: height=%d width=%d C.rpn_stride=%d'%(Y[0].shape[1], Y[0].shape[2], rpn_stride))
    print(X.shape)
    ...
    if debug_num_pos==0:
        ...
        for i in range(debug_num_pos):
            ...
            anchor_size = anchor_box_scales[int(idx/3)-1]
