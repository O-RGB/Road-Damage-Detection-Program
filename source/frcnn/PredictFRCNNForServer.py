...
from AI.source.frcnn import roi_helpers
import pandas as pd
...
class AI:
    ...
    def format_img_size(self,img, C): # utility function 1
    ...
    def preprocess_img(self,img, preprocessing_function): #utility function 2
    ...
    def format_img(self,img, C, preprocessing_function): # utility function 3
        """ formats an image for model prediction based on config """
        img, ratio = self.format_img_size(img, C)
        img = self.preprocess_img(img, preprocessing_function)
        return img, ratio

    # Method to transform the coordinates of the bounding box to its original size
    def get_real_coordinates(self,ratio, x1, y1, x2, y2): #utility function 4
    ...
    def CHECK_MODEL(self,test_images_list,network_arch,num_rois=32):
        ...
        classifier = nn.classifier(feature_map_input, roi_input, C.num_rois, len(self.class_mapping))
        ...
        self.model_classifier.load_weights(C.weights_all_path, by_name=True)


    def PREDECT_PHOTO(self, preprocessing_function = None,final_classification_threshold = 0.8):
        ...
        for idx, filepath in enumerate(self.test_images_list):
            print(os.path.basename(filepath))

            img = cv2.imread(filepath)
            imgReal = img.copy()

            X, ratio = self.format_img(img, self.C, preprocessing_function)

            # get the feature maps and output from the RPN
            [Y1, Y2, F] = self.model_rpn.predict(X)


            R = roi_helpers.rpn_to_roi(Y1, Y2, self.C, K.image_data_format(), overlap_thresh=self.C.rpn_nms_threshold,flag="test")
            ...
            for key in bboxes:
                ...
                new_boxes, new_probs = roi_helpers.non_max_suppression_fast(bbox, np.array(probs[key]), overlap_thresh=self.C.test_roi_nms_threshold,max_boxes=self.C.TEST_RPN_POST_NMS_TOP_N) #0.3 default threshold from original implementation
                for jk in range(new_boxes.shape[0]):
                    ...
                    elif key == 'repair': temp_all_if_repei.append([real_x1, real_y1, real_x2, real_y2])
...
#     list_of_all_images,final_df = Test_frcnn(network_arch = nn)
#     cv2.imwrite('test{}.png'.format(0),list_of_all_images[0])
